...
<?php
  ...
  $combo = pg_query($dbconn, "select id_especi, nombre from especialidad");

?>

<html>
   <head>
      <title>Especialidades de Medico</title>
   ...
   <body>
   <form name="frmAgregar" method="post" action="especialidadmedicograph.php">
   <div >				
   <select class="form-control" name = "slect" id="selctc" required>
        ...
        <?php
          while ($fila = pg_fetch_array($combo)){
            echo "<option value =".$fila['id_especi'].">".$fila['nombre']."</option>";
          }
          ?>
          </select>
          <input class="btn btn-primary" type="submit" name="btnAgregar" value="Consultar medicos en esta especialidad">
				</div>
<?php
    ...
    $result = pg_query($dbconn, "select nombre, mont_maxvent from cliente where id_especi=".$espec.";") or exit("Error with quering database");
    $result2 = pg_query($dbconn, "select max(mont_maxvent) from cliente where id_especi = ".$espec.";");    
  ...
  // creating an associative array to store the chart attributes
	$arrData = array(
        "chart" => array(
            ...
            "caption"=> "medicos por especialidad",
          	"captionFontSize"=> "24",
