...
<?php
  ...
  $combo = pg_query($dbconn, "select id_lote, lote from lote");

?>

<html>
   <head>
      <title>productos por lote</title>
   ...
   <body>
   <form name="frmAgregar" method="post" action="loteproductosgraph.php">
   <div >				
   <select class="form-control" name = "slect" id="selctc" required>
        ...
        <?php
          while ($fila = pg_fetch_array($combo)){
            echo "<option value =".$fila['id_lote'].">".$fila['lote']."</option>";
          }
          ?>
          </select>
          <input class="btn btn-primary" type="submit" name="btnAgregar" value="Consultar productos de esta lote">
				</div>
<?php
    ...
    $result = pg_query($dbconn, "select producto.nombre_producto, cantidad from lote inner join producto ON producto.id_producto = lote.id_producto
    where id_lote=".$espec.";") or exit("Error with quering database");
    $result2 = pg_query($dbconn, "select max(cantidad) from lote where id_lote = ".$espec.";");    
